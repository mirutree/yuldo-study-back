...
router.get("/:type/:search", async (req, res) => {
  try {
    ...
    if (!search) {
      res.status(401).json({ result: -1, data: null, err: "검색어를 입력 해주세요." });
    }
    ...
    if (result.length === 0) {
      res.status(200).json({ result: -2, data: null, err: "검색 결과가 없습니다." });
    } else {
      res.status(200).json({ result: 1, data: result });
    }
  } catch (e) {
